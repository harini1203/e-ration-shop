...
$time = $_POST['time'];
	...
	if($conn->connect_error){
		...
		$stmt = $conn->prepare("insert into nphh(rice,dhaal,sugar,oil,wheat,date,time) values(?,?,?,?,?,?,?)");
		$stmt->bind_param("iiiiiss", $rice,$dhaal,$sugar,$oil,$wheat,$date,$time);
		$execval = $stmt->execute();
		echo '<script>alert("Registration successfully...")</script>';
$sql = "SELECT id,rice,dhaal,sugar,oil,wheat,date,time FROM nphh ORDER BY id DESC LIMIT 1";
...
echo "<td>" .$row["dhaal"]*10 +$row["sugar"]*10+$row["oil"]*15+$row["wheat"]*20 ."</td>";
echo "</tr>";
echo "<td> Time </td>";
echo "<td>" . $row["time"] . "</td>";
echo "</tr>";
